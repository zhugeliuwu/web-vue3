...
const func = (root: HTMLDivElement, initTheme: string, changeTheme: { (detail: any): void; (arg0: string): void; }) => {
    const $ = (s) => {
    ...
    const moveElementRandomly = (element: { style: { transform: string; }; }) => {
...
class DayNightToggleButton extends HTMLElement {
    constructor() {
        ...
        const size = this.getAttribute("size") == null ? 1 : +this.getAttribute("size");
